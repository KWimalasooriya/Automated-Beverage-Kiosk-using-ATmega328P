...
// Function to turn on a motor with a given percentage
void turnOnMotor(uint8_t motor, uint8_t percentage) {
...
void turnOffMotors() {
    PORTD |= (1 << PD0) | (1 << PD1) | (1 << PD2) | (1 << PD3);  // Turn off motors
}

// Function to get delay for a given percentage
uint16_t getDelayForPercentage(uint8_t percentage) {
    switch (percentage) {
        case 0: return 0;      // 0% -> 0 seconds
        case 20: return 2180;  // 20% -> 2.18 seconds
        case 40: return 4110;  // 40% -> 4.11 seconds
        case 60: return 5730;  // 60% -> 5.73 seconds
        case 80: return 6970;  // 80% -> 6.97 seconds
        case 100: return 8110; // 100% -> 8.11 seconds
